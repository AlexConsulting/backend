  ...
  try {
    ...
    async function handleAnalyze() {
      ...
      try {
		...
		//const classification = scoreObj.classification || (scoreValue >= 70 ? "HIGH" : scoreValue >= 40 ? "MEDIUM" :  "LOW");
...
try {
  ...
  // ----- Contadores (malicious / suspicious) -----
  ...
  // ----- Engines que retornaram positivo (lista curta) -----
  const engines = vtObj.analysis?.data?.attributes?.results
               || vtObj.url_obj?.data?.attributes?.last_analysis_results
               || {};

  const positives = Object.entries(engines)
    .filter(([, v]) =>
      String(v?.result || v?.category || "").toLowerCase().includes("malicious") ||
      String(v?.result || "").toLowerCase().includes("suspicious")
    )
    .map(([k]) => k);

  if (positives.length) {
    evidenceList += `<li><strong>VT engines:</strong> ${escapeHtml(positives.join(", "))}</li>`;
  }
// ----- NOVO BLOCO: Security Vendors' Analysis (lista completa) -----

// Garantir que engines é um objeto válido
const vtResults =
...
if (vtResults) {
  ...
  // Severidade em ordem de prioridade
  const severityOrder = [
    "malicious",
    "malware",
    "phish",
  ...
  function severityIcon(status) {
    ...
    if (status.includes("malicious") || status.includes("malware")) return "🟥";
    if (
      status.includes("phish") ||
      status.includes("threat") ||
      status.includes("dangerous") ||
      status.includes("blocked") ||
      status.includes("blacklist")
    ) return "🟧";
    if (status.includes("suspicious")) return "🟨";

    return "⚪";
  }

  const entries = Object.entries(vtResults);

  // FILTRO: engines relevantes
  let relevantes = entries.filter(([engine, data]) => {
    const status = (data?.result || data?.category || "").toLowerCase();
    return severityOrder.some(key => status.includes(key));
  });
  ...
  if (relevantes.length > 0) {
    ...
    relevantes.sort((a, b) => {
      const aStatus = (a[1]?.result || a[1]?.category || "").toLowerCase();
      const bStatus = (b[1]?.result || b[1]?.category || "").toLowerCase();

      const aIndex =
        severityOrder.findIndex(s => aStatus.includes(s)) === -1
          ? 999
          : severityOrder.findIndex(s => aStatus.includes(s));

      const bIndex =
        severityOrder.findIndex(s => bStatus.includes(s)) === -1
          ? 999
          : severityOrder.findIndex(s => bStatus.includes(s));

      return aIndex - bIndex;
    });

    const vendorLines = relevantes
      .map(([engine, data]) => {
        const status = (data?.result || data?.category || "unknown");
        const icon = severityIcon(status);
        return `<li title="${escapeHtml(status)}">${icon} ${escapeHtml(engine)} — <strong>${escapeHtml(status)}</strong></li>`;
      })
      .join("");

    const count = relevantes.length;

    evidenceList += `
      <li>
        <strong>VT Return Security vendors' analysis (${count} detecções):</strong>
        <ul>${vendorLines}</ul>
      </li>
    `;
  }
...
} catch (errVendorBlock) {
  console.error("[VT Vendors] Erro interno no bloco de vendors:", errVendorBlock);
  // Continua normalmente sem quebrar a interface
}
